...
export default function Home() {
  ...
  const [newStaffDept, setNewStaffDept] = useState("家電"); // 部門
  ...
  useEffect(() => {
    ...
    fetchStaffs();
    fetchConfig(y, m);
    // 確定シフトは管理者のみが見るべきだが、データとしては取得しておく（表示制御で隠す）
  ...
  const handleAddStaff = async () => {
    ...
    // 店長ランクの場合は部門を強制的に「店長」扱いにしてもいいが、一旦入力通りにする
    // ただしソート順のために部門は重要
              ...
              <div className="mb-8">
                <div className="flex justify-between items-end mb-2">
                   ...
                   <div className="flex gap-2">
                     <button 
                       className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded shadow text-sm"
                       onClick={() => alert("現在、シフト作成はPC上のツール(make_shift.py)で行ってください。\n部門ルールを適用した最新版を実行してください。")}
                     >
                       ⚡ シフト作成(準備中)
                     </button>
                     {Object.keys(determinedSchedule).length > 0 && (
                       <button 
                         onClick={downloadCSV}
                         className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow flex items-center gap-2 text-sm"
                       >
                         📄 CSV出力
                       </button>
                     )}
                      ...
                      {getSortedStaffs().map((staff) => (
                        <tr key={staff.id} className="hover:bg-gray-50">
                          <td className="p-2 border font-bold text-left whitespace-nowrap sticky left-0 bg-white z-10">
                            {staff.name} <span className="text-[10px] text-gray-400 font-normal">({staff.department || "-"})</span>
                          </td>
                          {[...Array(daysInMonth)].map((_, i) => {
                             const d_str = str(i+1);
                             const dayData = determinedSchedule[d_str] || [];
                     ...
                     {[...Array(daysInMonth)].map((_, i) => {
                        ...
                        return (
                          <div key={d} className="bg-white border rounded p-1 flex flex-col items-center">
                            ...
                            <input type="number" className="w-full text-center border-b border-yellow-200 focus:outline-none bg-transparent" placeholder="0"
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded bg-gray-50 border">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                         <select className="border p-2 rounded flex-1" value={newStaffDept} onChange={e=>setNewStaffDept(e.target.value)}>
                            <option value="家電">家電</option>
                            <option value="季節">季節</option>
                            <option value="情報">情報</option>
                            <option value="通信">通信</option>
                            <option value="-">所属なし(店長等)</option>
                         </select>
...
function str(n) { return String(n); }"use client";
import { useState, useEffect } from "react";
import { db } from "../lib/firebase";
import { collection, addDoc, getDocs, deleteDoc, doc, updateDoc, query, orderBy, setDoc, getDoc } from "firebase/firestore";
